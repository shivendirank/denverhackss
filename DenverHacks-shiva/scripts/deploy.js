...
function getArtifact(contractName) {
    const artifactPath = resolve(".hardhat/artifacts/contracts", `${contractName}.sol`, `${contractName}.json`);
...
async function main() {
    ...
    if (!chain)
        throw new Error(`Chain not configured for: ${networkName}`);
    ...
    if (balance === 0n)
        throw new Error("Wallet has no funds on this chain!");
    const deployments = {
        network: networkName,
    ...
    if (config.contracts.includes("ToolRegistry"))
        deployments["toolRegistry"] = await deploy("ToolRegistry");
    if (config.contracts.includes("Escrow"))
        deployments["escrow"] = await deploy("Escrow", [account.address]);
    if (config.contracts.includes("UsageLog"))
        deployments["usageLog"] = await deploy("UsageLog", [account.address]);
    if (config.contracts.includes("AgentNFT"))
        deployments["agentNFT"] = await deploy("AgentNFT", [account.address]);
